...
class Lexer {
  ...
  private:
    ...
    /// the current line and request the next line buffer to the derived class
    /// as needed.
    int getNextChar() {
        // The current line buffer should not be empty unless it is the end of
        // file.
    ...
    /// just like getNextChar(), but won't actually consume char
    int peekNextChar() {
        if (curLineBuffer.empty())
            return EOF;
    ...
    Token getTok() {
        ...
        if (isalpha(lastChar)) {
            ...
            while (isalnum((lastChar = Token(getNextChar()))) ||
                   lastChar == '_')
                identifierStr += (char)lastChar;
        ...
        if (lastChar == '/' && peekNextChar() == '/') {
            std::ignore = Token(getNextChar()); // skip a '/'
